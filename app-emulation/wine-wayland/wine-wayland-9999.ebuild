...
if [[ ${PV} == *9999 ]]; then
	inherit git-r3
	EGIT_REPO_URI="https://gitlab.collabora.com/alf/wine.git"
	EGIT_BRANCH="wayland"
else
...
HOMEPAGE="https://gitlab.collabora.com/alf/wine/"
...
PATCHES=(
	"${FILESDIR}"/wine-vanilla-7.0-noexecstack.patch
	"${FILESDIR}"/wine-vanilla-7.20-unwind.patch
	"${FILESDIR}"/wine-vanilla-8.13-rpath.patch
	"${FILESDIR}"/fix-compattibility-with-libxkbcommon-1.6.0.patch
)
