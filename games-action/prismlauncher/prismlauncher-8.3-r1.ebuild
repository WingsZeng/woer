...
IUSE="debug lto qt6 test wayland"
REQUIRED_USE="
	lto? ( !debug )
"
...
DEPEND="
	...
	media-libs/libglvnd
	>=virtual/jdk-1.8.0:*
	wayland? ( >=media-libs/glfw-3.4-r1[wayland] )
"
...
src_prepare() {
	if use wayland; then
		PATCHES+=(
			${FILESDIR}/default-system-GLFW.patch
		)
	fi
	...
	if [[ ${java_version} -ge 20 ]]; then
		...
		eapply "${FILESDIR}/prismlauncher-8.2-openjdk21.patch"
	fi
